...
@add_arg_table! s begin
    ...
    default = "profiles/linear_reversible.jl"
    required = false
...
if calculate_flag
if isfile("matrix_data_spontaneous_linear.jld")
    d = load("matrix_data_spontaneous_linear.jld")
    M_internal_on = d["on"]
    ...
    save("matrix_data_spontaneous_linear.jld","on",M_internal_on,"off",M_internal_off,"docking_simple",M_docking_simple,"docking_H2d",M_docking_H2d,"docking_H3_H4",M_docking_H3_H4,"docking_HEX",M_docking_HEX,"undocking_simple",M_undocking_simple,"undocking_coupled",M_undocking_coupled,"association_H2d",M_association_H2d,"association_H3_H4",M_association_H3_H4,"association_HEX",M_association_HEX,"association_OCT",M_association_OCT,"dissociation",M_dissociation)
end
end
...
if calculate_flag
    ...
    for id in prog
        p = pars[id]
        k_association_OCT = exp(-2p.E_L - 2p.E_P_H2d - p.E_P_H3_H4) * p.k_on
        λ = λs(p)
        push!(result_df, (p.k_on,k_association_OCT, p.E_c, p.E_L, p.E_P_H2d, p.E_P_H3_H4, p.k_undocking_simple, p.k_undocking_coupled, λ[1] ,λ[2]))
        set_multiline_postfix(prog, "k_on = $(p.k_on)\n E_c = $(p.E_c)\n E_L = $(p.E_L)\n k_undocking_simple = $(p.k_undocking_simple)\n k_undocking_coupled = $(p.k_undocking_coupled)\n λ₀ = $(λ[1])\n λ₁ = $(λ[2]) ")
    end

    CSV.write("eigvals_model_linear_$(label).csv", result_df)
end
function λ̂₁(p::Par)
    k_association_OCT = exp(-2p.E_L - 2p.E_P_H2d - p.E_P_H3_H4) * p.k_on
    return minimum(
        [
            (N * k_association_OCT + N * exp(N*p.E_c) * p.k_on),
            2p.k_on * exp(p.E_c)
        ]
    )
end
...
for p in (pars)
    begin
        ...
        λ̂ = λ̂₁(p)
        push!(estimate_df, (p.k_on, k_association_OCT, p.E_c, p.E_L, p.E_P_H2d, p.E_P_H3_H4, p.k_undocking_simple, p.k_undocking_coupled, λ̂))
    end
end

CSV.write("eigvals_model_linear_$(label)_estimate.csv", estimate_df)
...
# CSV.write("eigvals_model_linear_$(label)_flow_parameter.csv", estimate_df)